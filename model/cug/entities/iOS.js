...
let iOS = (() => {
         ...
         * @returns {[]}
         */
        let getLevels = (composeLevel) => {
            let levels = [];
            let tem = composeLevel.split('-');
        ...
        let searchWebStyle = (webStyleGroup, level) => {
            webStyleGroup.forEach((item) => {
                ...
                //item.type == "style",在此寻找
                    ...
                    //  判断是复合层级还是单层级
                    //  图层id的array
                    let exitLayers = Object.keys(layerStack);
                    // 新图层样式
                    if (exitLayers.indexOf(item.name) == -1) {
                        let tem = createTem(item, level);
                        layerStack[item.name] = tem;
                    } else {
                        updateTem(layerStack[item.name], item, level);
                    }
                    //    添加
                ...
                switch (item.type) {
                    case "point-label":
                        ...
                        pointLabelPaints.forEach((paintStyle) => {
                            let attr = mapper.getSymbolPointAttrValue(paintStyle, item.styles);
                            if (attr !== "noAttr") {
                        ...
                        pointLabelLayouts.forEach((layoutStyle) => {
                            let attr = mapper.getSymbolPointAttrValue(layoutStyle, item.styles);
                            if (attr !== "noAttr") {
                    ...
                    case "line-label":
                        ...
                        lineLabelPaints.forEach((paintStyle) => {
                            let attr = mapper.getSymbolLineAttrValue(paintStyle, item.styles);
                            if (attr !== "noAttr") {
                        ...
                        lineLabelLayouts.forEach((layoutStyle) => {
                            let attr = mapper.getSymbolLineAttrValue(layoutStyle, item.styles);
                            if (attr !== "noAttr") {
                    ...
                    case  "polygon":
                        ...
                        polygonPaints.forEach((paintStyle) => {
                            let attr = mapper.getFillAttrValue(paintStyle, item.styles);
                            if (attr !== "noAttr") {
                        ...
                        polygonLayouts.forEach((layoutStyle) => {
                            let attr = mapper.getFillAttrValue(layoutStyle, item.styles);
                            if (attr !== "noAttr") {
                    ...
                    case "line":
                        ...
                        linePaints.forEach((paintStyle) => {
                            let attr = mapper.getLineAttrValue(paintStyle, item.styles);
                            if (attr !== "noAttr") {
